# **************************************************************************************
# parametrize pipeline design            # current is an impoved version of    C:/TEMP/GlobalBuckling/Global_Buckling2Kyriakides.py
# **************************************************************************************
...
if 'initialized' in globals():  # means this script is called as a function from another script
...
tubeModel.StaticStep(name='Weight', 
    ...
    nlgeom      = ON)
# Create the loading step
tubeModel.StaticStep(name='ThermalLoad', 
    previous='Weight', 
    timePeriod  = T['steps'][1]['timePeriod'],
    maxNumInc   = T['steps'][1]['maxNumInc'], 
    initialInc  = T['steps'][1]['initialInc'], 
    minInc      = T['steps'][1]['minInc'], 
    maxInc      = T['steps'][1]['maxInc'], 
    nlgeom      = ON)
region=tubeInstance.sets['X0']
tubeModel.steps['ThermalLoad'].Monitor(dof=3, node=region, 
    frequency=1)
# tubeModel.steps['ThermalLoad'].Restart(frequency=100, 
    # overlay=OFF)
region=tubeInstance.sets['X0']
tubeModel.steps['ThermalLoad'].Monitor(dof=3, node=region, 
    frequency=1)
...
tubeModel.HistoryOutputRequest(name='Temperature', 
    createStepName='ThermalLoad', variables=('NT', 'U3', ), region=regionDef, 
